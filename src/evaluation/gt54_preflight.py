"""GT54 dataset inventory and fail-closed cross-tile grid preflight.

This module only inspects raster metadata and GT masks.  It never reprojects,
resamples, or writes raster data, and it does not inspect model predictions.
"""
...
def preflight_grids(inventory: Iterable[RasterMetadata],
    ...
    return PreflightResult(not errors, tuple(metadata), tuple(placements), tuple(rows),
...
def write_preflight_outputs(output_dir: Path, result: PreflightResult,
    ...
    summary = [{"status": "PASS" if result.passed and not error else "FAIL",
                ...
                "error_count": len(result.errors) + bool(error),
                "errors": " | ".join((*result.errors, *((error,) if error else ()))),
                **tolerance_values}]
