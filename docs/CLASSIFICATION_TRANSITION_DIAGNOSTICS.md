...
`tools/evaluation/compare_classification_rasters.py` は、同じグリッド上にある2枚の1-band OEM8分類GeoTIFFを
...
```bash
python -m tools.evaluation.compare_classification_rasters \
...
```bash
python -m tools.evaluation.compare_classification_rasters 02.tif 03.tif \
  --label 02_to_03_aoi \
...
```bash
python -m tools.evaluation.compare_classification_progression \
